...
function createBlockMod(index_p, index_l, p::parms, s::sets)
    ...
    @variable(m, r_ladd_d_[i=P, j=Y, l=L, (0,1)] >= 0)
    ...
    @variable(m, n_ladd_d_[i=P, j=Y, l=L, (0,1)] >= 0)
    @variable(m, n_ladd[i=P, j=Y, l=L])

    @variable(m, n_l_pd_[i=P, j=Y, l=L, (0,1)])
    ...
    @constraint(m, e_l_s_e_[i=P, j=Y, l=L],
                e_l[i, j, l] == 
                #sum(e_l_pd_[i, j, l, k] for k in (0, 1))
    ...
    @constraint(m, r_eh_d_e_[i=P, j=Y, l=L, k=Kr],
                r_eh_d_[i, j, l, k] == p.r_c_H[l+1, k+1] * r_cp_d_[i, j, l, k] 
    ...
    @constraint(m, r_ehf_d_e_[i=P, j=Y, l=L, k=Kr, f=Fu],
                r_ehf_d_[i, j, l, k, f] ==
                p.r_c_F[l+1, k+1, f+1] * r_eh_d_[i, j, l, k] 
                + p.r_rhs_F[l+1, k+1, f+1] * y_r[i, j, l, k]
               )
    # r_ehf_bM
    ...
    @constraint(m, r_u_d_e_[i=P, j=Y, l=L, k=Kr],
                r_u_d_[i, j, l, k] == 
                (1 - p.r_c_UonSite[i+1, j+1, l+1, k+1])
    ...
    @constraint(m, r_ep0_d_e_[i=P, j=Y, l=L, k=Kr],
                r_ep0_d_[i, j, l, k] == 
                # fuel
                sum(p.r_c_Fe[l+1, k+1, f+1]*r_ehf_d_[i, j, l, k, f] for f in Fu) 
                # process
                + r_cp_e_d_[i, j, l, k]
                # electricity
                + sum(p.r_c_Fe[l+1, k+1, f+1]
                    *p.r_c_Hr[l+1, k+1, f+1]
                    *p.r_c_Fgenf[l+1, k+1, f+1]
    ...
    @constraint(m, m_l_s_e_[i=P, j=Y, l=L], # slack (1) is necessary here
                r_l[i, j, l] == sum(r_l_pd_[i, j, l, k] for k in (0, 1))
               )
    ...
    @constraint(m, n_cap_add_d_m_i_[i=P, j=Y, l=L, k=Kn],
                n_c0_d_[i, j, l, k] <= p.n_c0_bM * y_n[i, j, l, k]
               )
    #
    ...
    @constraint(m, n_ladd_m_1_y_i_[i=P, j=Y, l=L; j<n_years],
                n_l_pd_[i, j, l, 1] <= 
    ...
    @constraint(m, n_l_s_e_[i=P, j=Y, l=L],
                n_l[i, j, l] ==
    ...
    @constraint(m, n_eh_d_e_[i=P, j=Y, l=L, k=Kn],
                n_eh_d_[i, j, l, k] == p.n_c_H[l+1, k+1] * n_cp_d_[i, j, l, k] 
    ...
    @constraint(m, n_ehf_d_e_[i=P, j=Y, l=L, k=Kn, f=Fu],
                n_ehf_d_[i, j, l, k, f] ==
                p.n_c_F[l+1, k+1, f+1] * n_eh_d_[i, j, l, k] 
                + p.n_rhs_F[l+1, k+1, f+1] * y_n[i, j, l, k]
               )
    # n_ehf_bM
    ...
    @constraint(m, n_u_d_e_[i=P, j=Y, l=L, k=Kn],
                n_u_d_[i, j, l, k] == 
                (1 - p.n_c_UonSite[i+1, j+1, l+1, k+1])*
    ...
    @constraint(m, n_ep0_d_e_[i=P, j=Y, l=L, k=Kn],
                n_ep0_d_[i, j, l, k] == 
                # fuel
                sum(p.n_c_Fe[l+1, k+1, f+1]*n_ehf_d_[i, j, l, k, f] for f in Fu) 
                # process
                + n_cp_e_d_[i, j, l, k]
                # electricity
                + sum(p.n_c_Fe[l+1, k+1, f+1]
                      *p.n_c_Hr[l+1, k+1, f+1] 
                      *p.n_c_Fgenf[l+1, k+1, f+1]
...
function attachFullObjectiveBlock(index_p, index_l, p::parms, s::sets)
    ...
    @objective(m, Min,
               ...
               + sum(sum(sum(p.discount[i+1, j+1]*t_ret_cost[i, j, l] 
                             for l in L) for j in Y) for i in P)
               + sum(p.discount[n_periods+1, n_years+1]*o_loan_last[l,0] 
                     for l in L)

               # if you retire but still have unpayed loan it is gonna be
               # reflected here :()
              )
    @printf "Full objective must be attached\n" 
end
...
function attachLocationBlock(m::Model, p::parms, s::sets)
    ...
    @constraint(m, ag_co2_l_link_i_[i=P, j=Y],
                # existing
                -sum(o_ep0[i, j, l] for l in L) 
                # new
                - sum(n_ep0[i, j, l] for l in L)
                # grid associated emissions
                - sum(p.GcI[i+1,j+1,l+1]*(o_u[i, j, l] + n_u[i, j, l])
                                          for l in L)
